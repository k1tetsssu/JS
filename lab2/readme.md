...
|-----------|---------|----------------------|
| `map()`   | Создает новый массив, применяя функцию к каждому элементу. | `array.map(obj => ({ ...obj, age: obj.age + 1 }))` |
| `filter()` | Возвращает новый массив с элементами, удовлетворяющими условию. | `array.filter(obj => obj.age > 18)` |
| `reduce()` | Сворачивает массив в одно значение (объект, число, строку и т. д.). | `array.reduce((sum, obj) => sum + obj.age, 0)` |
...
// map() – увеличиваем возраст
const updatedUsers = users.map(user => ({ ...user, age: user.age + 1 }));

// filter() – выбираем пользователей старше 30 лет
const filteredUsers = users.filter(user => user.age > 30);

// reduce() – считаем сумму всех возрастов
